...
Solution.longestSlideDown_01 = {
  d: `intuitive`,
  f: function (pyramid) {
    pyramid = pyramid.map(x => x.slice(0));
...
Solution.longestSlideDown_03 = {
  d: ``,
  f: function (pyramid) {
    return pyramid.reduceRight((t, c) => c.map((n, i) =>
      t[i] > t[i + 1] ? n + t[i] : n + t[i + 1]
    ))[0];
  }
};

Solution.longestSlideDown_04 = {
  d: ``,
  f: function (pyramid) {
    let fold = (a, b) =>
      b.map((e, i) => Math.max(a[i - 1] || 0, a[i] || 0) + e);
    return Math.max(...pyramid.reduce(fold));
  }
};
